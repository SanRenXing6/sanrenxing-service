...
public class UserDataMysqlService implements UserDao {
    private final JdbcTemplate jdbcTemplate;

    @Autowired
    public UserDataMysqlService(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    @Override
    public int addUser(User user) {
        UUID id = UUID.randomUUID();  // Continue using UUID in Java and store it as CHAR(36) in MySQL
    ...
    public Optional<User> getUser(UUID id) {
        ...
        try {
            User user = jdbcTemplate.queryForObject(
                    sql,
                    new Object[]{id.toString()},  // Convert UUID to string for query
                    (resultSet, i) -> {
                        UUID userId = UUID.fromString(resultSet.getString("id"));  // Convert string back to UUID
                        String name = resultSet.getString("name");
                        String password = resultSet.getString("password");
                        String email = resultSet.getString("email");
                        UserStatus status = UserStatus.valueOf(resultSet.getString("status"));
    ...
    public Optional<User> getUserByEmail(String targetEmail) {
        ...
        try {
            User user = jdbcTemplate.queryForObject(
                    sql,
                    new Object[]{targetEmail},  // The email parameter is directly used here
                    (resultSet, i) -> {
                        UUID userId = UUID.fromString(resultSet.getString("id"));  // Convert string back to UUID
                        String name = resultSet.getString("name");
                        String password = resultSet.getString("password");
